...
if (Sys.info()[[1]]=="Windows") {
    ...
    #     jsonlite::fromJSON() |>
    #     as.data.table()
    confidential <- readxl::read_xlsx("./../data/secret/key4exp.xlsx") |> as.data.table()
    ...
    confidential <- Sys.getenv("DATABASE_EXP") |>
         base64enc::base64decode() |>
         rawToChar() |>
         jsonlite::fromJSON() |>
         as.data.table()
...
if (dim(sensors_new)[1]>1) {
    ...
    variables_old_head <- dbGetQuery(con, "SELECT * FROM \"Purple_Air_Exp\" LIMIT 5;")
    ...
    dbWriteTable(con,"Purple_Air_Exp",sensors_new, append = TRUE)
    cat("Done!\n")
} else {
